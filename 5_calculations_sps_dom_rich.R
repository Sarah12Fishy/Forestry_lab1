...
#----------------
tree_cnt <- trees %>% group_by(Plot, Chojnacky_Code) %>%
  tally()
#----------------
...
# No. dom_cnt contains only the rows where a species is dominant in a plot.
# If there are plots without a single dominant species or with ties in counts, dom_cnt might have fewer rows than sum_u2. 
...
#----------------
tree_total <- tree_cnt %>% group_by(Plot) %>%
  summarise(Ttl_Trees = sum(n, na.rm = TRUE))

richness <- tree_cnt %>% group_by(Plot) %>%
...
#----------------
dom_cnt <- dom_cnt %>% left_join(tree_total, by = "Plot") %>%
...
# Question: What’s the most abundant species at plot D5? Chojnacky

most_abundant_species_D5 <- dom_cnt %>%
  filter(Plot == "D5")

print(most_abundant_species_D5)
...
# Question 7: How many trees are there at plot A5? 43

# Question 8: How many different tree species are there at plot D1? 60 trees.

species_D1 <- tree_total %>%
  filter(Plot == "D1")
print(species_D1)
