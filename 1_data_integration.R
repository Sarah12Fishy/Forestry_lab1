...
#----------------
u2_data <- read.csv("https://raw.githubusercontent.com/DS4Ag/Forestry_lab1/main/U2_2017data.csv")

biomass_data <- read.csv("https://raw.githubusercontent.com/DS4Ag/Forestry_lab1/refs/heads/main/Biomass%20Equation.csv")
...
#----------------
clean_u2_data <- u2_data %>% filter(!is.na('DBH') & !is.na('Code'))

clean_species_data <- species_data %>% filter(!is.na('DBH') & !is.na('Code'))
  
clean_biomass_data <- biomass_data %>% filter(!is.na('DBH') & !is.na('Code'))
#----------------
...
#----------------
overstory_trees <- clean_u2_data %>% filter(Class == "O")
