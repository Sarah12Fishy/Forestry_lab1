...
#----------------
trees <- trees %>% mutate(DBH_cm = DBH * 2.54) %>%
  mutate(biomass = exp(b0 + b1 * log(DBH_cm)))
#----------------
...
#----------------
sum_u2_bm <- sum_u2_bm %>% mutate(biomass_per_acre = total_biomass * EF)
#----------------
...
# YOUR ANSWER The plot with the highest amount of biomass is J6.

#----------------

highest_biomass_plot <- sum_u2 %>%
  arrange(desc(biomass_per_acre)) %>%
  slice(1) %>%
  select(Plot)

print(highest_biomass_plot)
...
head(sum_u2 %>% arrange(desc(biomass_per_acre)))
