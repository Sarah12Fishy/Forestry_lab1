...
#----------------
sum_u2 <- sum_u2 %>% left_join(dom_cnt, by = "Plot") %>%
...
#----------------
sum_u2 <- sum_u2 %>%
  left_join(species_info, by = c("Chojnacky_Code" = "Code"))

sum_u2 <- sum_u2 %>%
  rename(Dom_species = Chojnacky_Code, Abundance = n)
...
#----------------
sum_u2 <- sum_u2 %>%
  mutate(biomass_tons_per_acre = biomass_per_acre / 1000)
...
# 23 now is the dominate species. 

dominant_species_D5_updated <- sum_u2 %>%
  filter(Plot == "D5") %>%
  select(Dom_species)

print(dominant_species_D5_updated) 
